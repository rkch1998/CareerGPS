...
public class QuestionService {
  ...
  public List<QuestionDTO> getActiveQuestions() {
    return questionRepository.findByActiveTrueOrderByDisplayOrderAsc().stream()
        .map(this::toDto)
        .toList();
  }
  ...
  private QuestionDTO toDto(Question question) {
    var options =
        optionRepository
            .findByQuestionQuestionIdOrderByDisplayOrderAsc(question.getQuestionId())
            .stream()
            .map(
                option ->
                    new QuestionOptionDTO(
                        option.getOptionId(), option.getOptionText(), option.getDisplayOrder()))
            .toList();
