...
public class RecommendationService {
  ...
  public List<RecommendationDTO> recommend(Long assessmentId) {
    if (!assessmentRepository.existsById(assessmentId))
      throw new EntityNotFoundException("Assessment " + assessmentId + " was not found");
    if (answerRepository.findByAssessmentId(assessmentId).isEmpty())
      throw new IllegalStateException(
          "Answer at least one question before viewing recommendations");
    Map<Long, Integer> traitScores = studentTraitScores(assessmentId);
    List<ScoredCareer> scored =
        careerRepository.findAll().stream()
            .map(career -> score(career, traitScores))
            .filter(item -> item.score > 0)
            .sorted(
                Comparator.comparingInt(ScoredCareer::score)
                    .reversed()
    ...
    for (int index = 0; index < scored.size(); index++) {
      ...
      results.add(
          new RecommendationDTO(
              level,
              item.career.getCareerId(),
              item.career.getName(),
              item.career.getDescription(),
              item.reasons));
    }
    return results;
  }
    ...
    return jdbcTemplate
        .query(
            """
                ...
                """,
            (rs, row) -> Map.entry(rs.getLong("trait_id"), rs.getInt("score")),
            assessmentId)
        .stream()
    ...
    List<TraitWeight> weights =
        jdbcTemplate.query(
            """
                ...
                """,
            (rs, row) ->
                new TraitWeight(rs.getLong("trait_id"), rs.getInt("weight"), rs.getString("name")),
            career.getCareerId());
    int score = 0;
    List<String> reasons = new ArrayList<>();
    for (TraitWeight weight : weights) {
      if (studentTraitScores.getOrDefault(weight.traitId, 0) > 0) {
        score += studentTraitScores.get(weight.traitId) * weight.weight;
        reasons.add("You showed " + weight.name().toLowerCase() + " strengths");
      }
    }
