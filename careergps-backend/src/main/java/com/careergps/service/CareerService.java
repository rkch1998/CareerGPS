...
public class CareerService {
  ...
  public List<CareerDTO> getAllCareers() {
    return careerRepository.findAll().stream().map(this::convertToDTO).toList();
  }

  public Optional<CareerDTO> getCareerById(Long careerId) {
    return careerRepository.findById(careerId).map(this::convertToDTO);
  }

  public List<CareerDTO> getCareersByCategory(Long categoryId) {
    return careerRepository.findByCareerCategoryCategoryIdOrderByNameAsc(categoryId).stream()
